...
2. Immutable Creation: Use ONLY Loom generate commands for modules and resources. Never modify `src/index.ts` manually.
...
- `bun loom generate module <name>`: Create a CSS module and auto-register it.
- `bun loom generate resource <name> --field <name:type>`: Create a typed CRUD resource from deterministic field specs.
- `bun loom generate guard <name>`: Create an Elysia guard plugin (derive/resolve scoped).
- `bun loom generate middleware <name>`: Create an Elysia lifecycle middleware plugin.
- `bun loom generate hook <name>`: Create an Elysia macro hook plugin.
- `bun loom generate plugin <name>`: Create a generic Elysia plugin.
- `bun loom init swagger`: Wire @elysiajs/swagger into src/index.ts.
- `bun loom init env`: Wire environment configuration plugin.
- `bun loom init auth`: Wire bearer authentication guard and plugin.
- `bun loom init observability`: Wire structured logging plugin.
- `bun loom plan resource <name>`: Preview generated resource writes (dry-run).
- `bun loom validate resource <name>`: Validate resource specs without writing.
...
- `bun loom skeleton`: Refresh `.loom/context/skeleton.md`.
- `bun loom skeleton --json`: Refresh `.loom/context/skeleton.md` and `.loom/context/skeleton.json`.
...
- `--dry-run`: Preview supported write commands without changing files.
